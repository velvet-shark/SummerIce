...
class APIClient {
  ...
  createGeminiRequest(prompt, model, maxTokens) {
    return {
      ...
      generationConfig: {
        maxOutputTokens:
          maxTokens || CONFIG.LLM_PROVIDERS.GEMINI.models[model]?.maxTokens || 8192,
        temperature: 0.7
      }
    };
  }
  ...
  estimateMaxContentChars(maxTokens) {
    ...
    const availableTokens = Math.max(
      ...
      maxTokens - chunkConfig.OUTPUT_TOKEN_RESERVE - chunkConfig.PROMPT_TOKEN_OVERHEAD
    );
  ...
  splitContentIntoChunks(content, maxChunkChars, overlapChars) {
    ...
    while (start < content.length) {
      ...
      const boundaryOffset =
        boundary > Math.floor(maxChunkChars * 0.5) ? boundary + 1 : slice.length;
  ...
  buildChunkPrompt(chunk, chunkIndex, totalChunks, targetWords, summaryFormat) {
    const formatInstruction =
      summaryFormat === "bullets"
        ? "Format the summary as clear bullet points."
        : "Format the summary in concise paragraphs.";

    return `Summarize the section below from a longer article. Keep the key facts and context. Target about ${targetWords} words. ${formatInstruction}
...
---`;
  ...
  buildSynthesisPrompt(chunkSummaries, length, summaryFormat) {
    ...
    return `The text below contains summaries of multiple sections from one long article. Synthesize them into a single coherent summary around ${wordCount} words. ${formatInstruction} Remove duplication and keep the most important points.
...
---`;
  ...
  async requestSummary(prompt, settings, providerConfig, maxTokensOverride, retryCount = 0) {
    try {
      const { provider, model, apiKey } = settings;
      const maxTokens = maxTokensOverride || providerConfig.models[model].maxTokens;
      ...
      if (retryCount < CONFIG.RETRY_ATTEMPTS - 1) {
        ...
        return this.requestSummary(prompt, settings, providerConfig, maxTokensOverride, retryCount + 1);
      }
  ...
  async summarizeWithChunking(content, settings, providerConfig) {
    ...
    const chunks = this.splitContentIntoChunks(content, maxChunkChars, overlapChars);

    if (chunks.length <= 1) {
      const prompt = getSummaryPrompt(content, settings.summaryLength, settings.summaryFormat);
      return this.requestSummary(prompt, settings, providerConfig, maxTokens);
    }

    const wordCount = CONFIG.SUMMARY_LENGTHS[settings.summaryLength]?.words || 200;
    const targetWords = Math.max(80, Math.round(wordCount / Math.min(chunks.length, 6)));
    ...
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      const summary = await this.requestSummary(prompt, settings, providerConfig, chunkMaxTokens);
    ...
    return this.requestSummary(synthesisPrompt, settings, providerConfig, maxTokens);
  }
  ...
  async callAPI(content) {
    ...
    if (this.shouldChunkContent(content, maxTokens)) {
      return this.summarizeWithChunking(content, resolvedSettings, providerConfig);
    }

    const prompt = getSummaryPrompt(content, summaryLength, summaryFormat);
    return this.requestSummary(prompt, resolvedSettings, providerConfig, maxTokens);
  }
